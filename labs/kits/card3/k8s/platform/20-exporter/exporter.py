#!/usr/bin/env python3
"""Dependency-free OpenMetrics live replay exporter for Card 3."""
...
def load_inputs() -> tuple[dict, dict[str, str]]:
    data_path = Path(os.environ.get("REPLAY_DATA", "/data/card3_live_replay.json"))
    meta_path = Path(os.environ.get("METRIC_META", "/data/card3_metric_meta.json"))
...
def render_metrics() -> bytes:
    ...
    labels = ",".join(f'{key}="{escape_label(value)}"' for key, value in sorted(DATA["labels"].items()))
    ...
    for name, spec in DATA["metrics"].items():
        ...
        lines.append(f"# HELP {name} {escape_help(DESCRIPTIONS.get(name, name))}")
        lines.append(f"# TYPE {name} {metric_type}")
        lines.append(f"{name}{{{labels}}} {value:g}")
    lines.extend(
        (
            "# HELP card3_replay_elapsed_seconds Seconds elapsed since this exporter process started.",
            "# TYPE card3_replay_elapsed_seconds gauge",
            f"card3_replay_elapsed_seconds {elapsed:.3f}",
            "# HELP card3_replay_index Current zero-based replay data index.",
            "# TYPE card3_replay_index gauge",
            f"card3_replay_index {index}",
            "# EOF",
        )
    )
    return ("\n".join(lines) + "\n").encode("utf-8")
...
class Handler(BaseHTTPRequestHandler):
    server_version = "card3-live-replay/1.0"
...
def main() -> None:
    ...
    print(
        f"card3 live replay listening on {host}:{port}; mode={MODE}; "
