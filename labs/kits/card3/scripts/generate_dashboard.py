#!/usr/bin/env python3
"""Generate the student Grafana dashboard from card3_v2_data.json metadata."""
...
META_PATH = ROOT / "data" / "card3_metric_meta.json"
OUTPUTS = (
    ROOT / "grafana" / "dashboards" / "card3-student.json",
    ROOT / "k8s" / "platform" / "40-grafana" / "card3-student.json",
)
INTEGRATED_OUTPUT = ROOT / "k8s" / "overlays" / "integrated" / "platform" / "card3-student.json"
STANDALONE_DATASOURCE = {"type": "prometheus", "uid": "card3-prometheus"}
BAKED_DATASOURCE = {"type": "prometheus", "uid": "card3-baked"}
LIVE_DATASOURCE = {"type": "prometheus", "uid": "${card3_live_ds}"}
...
GROUPS = (
    (
        ...
        (
            ...
            "spark_stage_shuffleReadBytes_rate",
            "airflow_task_instance_running",
            "dataset_freshness_lag_minutes",
            "pipeline_retry_ratio_5m",
        ),
    ),
    (
        "DW 자원과 대화형 쿼리",
        (
            "dw_scan_bytes_rate",
            "container_memory_working_set_ratio",
            "trino_execution_running_queries",
            "trino_execution_queued_queries",
            "trino_query_wallTime_p95_seconds",
        ),
    ),
    (
        "용량과 장기 상태",
        (
            "cluster_autoscaler_nodes",
            "warehouse_storage_used_ratio",
        ),
    ),
    (
        "배포 관측: 시나리오와 실제 EKS",
...
def make_panel(metric: dict, panel_id: int, x: int, y: int, integrated: bool) -> dict:
    name = metric["name"]
    scenario_expr = f'{name}{{cluster="dw-01",job="dw-platform"}}'
    if integrated:
        targets = [
            target(scenario_expr, "과거 bake · dw-01", "A", BAKED_DATASOURCE),
            target(scenario_expr, "라이브 replay · dw-01", "B", LIVE_DATASOURCE),
        ]
        ...
        targets = [target(scenario_expr, "시나리오 · dw-01", "A", STANDALONE_DATASOURCE)]
        infrastructure_ref = "B"
        infrastructure_datasource = STANDALONE_DATASOURCE
        infrastructure_namespace = "dw-observability"
        infrastructure_deployment_match = ',deployment=~"card3-.*"'
...
def make_dashboard(source: dict, integrated: bool) -> dict:
    ...
    if len(metrics) != 14 or set(expected) != set(metrics):
        raise SystemExit("dashboard groups must contain each of the 14 metadata metrics exactly once")
    ...
    templating = {"list": []}
    description = "카드3 배치·DW 관측 실습 학생용 대시보드"
    if integrated:
