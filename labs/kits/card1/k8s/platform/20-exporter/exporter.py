#!/usr/bin/env python3
"""Dependency-free OpenMetrics live replay exporter for Card 1."""
...
def load_inputs() -> tuple[dict, dict[str, str]]:
    data_path = Path(os.environ.get("REPLAY_DATA", "/data/card1_live_replay.json"))
    meta_path = Path(os.environ.get("METRIC_META", "/data/card1_metric_meta.json"))
...
def render_metrics() -> bytes:
    ...
    lines.extend(
        (
            "# HELP card1_replay_elapsed_seconds Seconds elapsed since this exporter process started.",
            "# TYPE card1_replay_elapsed_seconds gauge",
            f"card1_replay_elapsed_seconds {elapsed:.3f}",
            "# HELP card1_replay_index Current zero-based replay data index.",
            "# TYPE card1_replay_index gauge",
            f"card1_replay_index {index}",
            "# EOF",
        )
    )
    return ("\n".join(lines) + "\n").encode("utf-8")
...
class Handler(BaseHTTPRequestHandler):
    server_version = "card1-live-replay/1.0"
...
def main() -> None:
    ...
    print(
        f"card1 live replay listening on {host}:{port}; mode={MODE}; "
