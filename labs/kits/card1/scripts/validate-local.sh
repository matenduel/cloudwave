...
VALIDATE_DIR=$(mktemp -d /private/tmp/card1-kit-static.XXXXXX)
...
replay = json.loads((root / "data/card1_live_replay.json").read_text())
meta = json.loads((root / "data/card1_metric_meta.json").read_text())
assert replay["labels"] == {"cluster": "commerce-01", "job": "commerce-platform"}
assert replay["step"] == 30
assert len(replay["metrics"]) == len(meta["metrics"]) == 21
...
for path, uid in (
    (root / "grafana/dashboards/card1-student.json", "card1-prometheus"),
    (root / "k8s/platform/40-grafana/card1-student.json", "card1-prometheus"),
    (root / "k8s/overlays/integrated/platform/card1-student.json", "-- Mixed --"),
):
...
env = os.environ.copy()
os.environ.update(REPLAY_DATA=str(root / "data/card1_live_replay.json"), METRIC_META=str(root / "data/card1_metric_meta.json"))
module = runpy.run_path(str(root / "k8s/platform/20-exporter/exporter.py"))
payload = module["render_metrics"]().decode()
assert 'http_requests_total{route="/static",cluster="commerce-01",job="commerce-platform"}' in payload
assert payload.endswith("# EOF\n")
os.environ.clear(); os.environ.update(env)
for name in ("history", "full"):
    path = root / f"k8s/bake/data/card1_bake_{name}.openmetrics.gz"
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        text = handle.read()
    assert text.endswith("# EOF\n")
    assert 'cluster="commerce-01",job="commerce-platform"' in text
print("replay/meta: 21 metrics, 121 points; dashboards: 21 time-series panels; gzip/exporter: PASS")
PY
...
service = find_one.call("Service", "card1-live-replay")
monitor = find_one.call("ServiceMonitor", "card1-live-replay")
...
datasource = find_one.call("ConfigMap", "card1-baked-datasource")
dashboard = find_one.call("ConfigMap", "card1-student-dashboard")
...
JSON.parse(dashboard.dig("data", "card1-student.json"))
...
echo "CARD1_STATIC_VALIDATION_PASS"
